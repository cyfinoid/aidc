...
aidc::destroy_scaffold() {
  ...
  # aidc-created files inside .devcontainer/ that are deliberately NOT in
  # AIDC_MANAGED_PATHS: project-setup.sh is user-owned (copy_template_once —
  # listing it as managed would make `init` treat an edited copy as a conflict),
  # and .env (plus its atomic-write temp) is regenerated by every
  # up/rebuild/rescan. Both live inside the directory `--purge-scaffold`
  # documents as removed (docs/install.md), so they go with it — otherwise the
  # non-empty rmdir below silently no-ops and the whole .devcontainer/ tree
  # survives the purge.
  rm -f "${workspace:?}/.devcontainer/project-setup.sh" \
        "${workspace:?}/.devcontainer/.env"
  rm -f "${workspace:?}"/.devcontainer/.env.aidc-tmp.* 2>/dev/null || true

  # .ai-container/ is aidc's own per-project state directory: project.env,
  # upgrade backups/, scan-hook.log, stamp temp files — all aidc's, and the
  # whole directory is git-excluded. --purge-scaffold is documented to remove
  # it, settings included (see the hint in config.sh). Remove the tree rather
  # than enumerating its contents: enumerating is exactly what left backup/ and
  # scan-hook.log behind, and any state file added later would be missed again.
  rm -rf "${workspace:?}/.ai-container"

  # rmdir, not rm -rf, for the directories that can legitimately hold files
  # that are not aidc's: a devcontainer asset or a Cursor rule you added is
  # yours, and the directory should survive rather than be deleted silently.
