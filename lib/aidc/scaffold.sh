...
aidc::destroy_scaffold() {
  ...
  # and .env is regenerated by every up/rebuild/rescan. Both live inside the
  # directory `--purge-scaffold` documents as removed (docs/install.md), so they
  # go with it — otherwise the non-empty rmdir below silently no-ops and the
  # whole .devcontainer/ tree survives a purge.
  rm -f "${workspace:?}/.devcontainer/project-setup.sh" \
        "${workspace:?}/.devcontainer/.env"
  # rmdir, not rm -rf: anything else the user put in .devcontainer/ is theirs,
  # and a non-empty directory should survive rather than be deleted silently.
